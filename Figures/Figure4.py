...
def figure4a(workdir, samples):
    ...
    for idx, plat in enumerate(['HiFi', 'ONT']):
        print('# DEL invalid: {0}/{1} ({2}) {3}'.format(invalid_counter[plat]['del'], valid_total[plat]['del'],
                                                    invalid_counter[plat]['del'] / valid_total[plat]['del'] * 100, uniques_total[plat]['del']))
        print('# INS invalid: {0}/{1} ({2}) {3}'.format(invalid_counter[plat]['ins'], valid_total[plat]['ins'],
                                                    invalid_counter[plat]['ins'] / valid_total[plat]['ins'] * 100, uniques_total[plat]['ins']))
        ax = axes[idx]
        ax.bar([0, 1], [uniques_total[plat]['del'], uniques_total[plat]['ins']], facecolor='#bfbdbe', edgecolor='w', width=width)
        ax.bar([0, 1], [valid_total[plat]['del'], valid_total[plat]['ins']], facecolor='#bfcad6', edgecolor='w', width=width)
        ax.bar([0, 1], [invalid_counter[plat]['del'], invalid_counter[plat]['ins']], facecolor='#fbb4ae', edgecolor='w', width=width)
...
def figure3b(workdir, samples):
    ...
    for plat, dataset in dataset_dict.items():
        for svtype in ['del', 'ins']:
            svs_info = {}
            for line in open(f'{workdir}/{plat}/stra_compare_hq/ExTD/{dataset}_{svtype}_assm_read_merged.ExTD.vcf', 'r'):
                if '#' in line:
                    continue
                entries = line.strip().split('\t')
            ...
            df_unique_svs = pd.read_csv(f'{workdir}/{plat}/stra_compare_hq/ExTD/{dataset}.{svtype}.assm-uniques.ExTD.bed', sep='\t', names=['chrom', 'start', 'end', 'id', 'svtype'])
            uniques_total[plat][svtype] = len(df_unique_svs)

            sv_valid = {}
            for sample_idx, sample in enumerate(samples):
                with open(f'{workdir}/{plat}/stra_compare_hq/ExTD/trio_validation/ttmars_combined_res.{svtype}.{sample}.txt', 'r') as f:
                    for line in f:
                        entries = line.strip().split('\t')
...
def figure4c(workdir):
    ...
    for plat, dataset in dataset_dict.items():
        assm_invalid = pd.read_csv(f'{workdir}/{plat}/stra_compare_hq/ExTD/trio_validation/ttmars_trio_invalid_insdel.tsv', sep='\t', names=columns)
        ...
        for line in open(f'{workdir}/{plat}/stra_compare_hq/ExTD/trio_validation/vapor_trio_invalid_insdel.tsv', 'r'):
            entries = line.strip().split('\t')
    ...
    for idx, plat in enumerate(['HiFi', 'ONT']):
        ...
        sns.lineplot(data=plat_invalid, x='reg', y='pcrt', hue='stra', style='svtype', ci=None,
                     hue_order=['Assembly', 'Read'], palette=[STRACOLORS['Assembly'], STRACOLORS['Read']], lw=2, markers=['o', 'X'], markersize=9, ax=ax)
...
def figure4d(workdir, plat):

    columns = ['chrom', 'start', 'end', 'id', 'svtype', 'svlen', 'region', 'reptype', 'pcrt', 'mapq', 'start_diff_std', 'size_std', 'size_mean', 'sigs']


    dissize = open(f'{workdir}/{plat}/stra_compare_hq/ExTD/trio_validation/ttmars_trio_invalid_insdel.discsize.tsv','w')
    dispos = open(f'{workdir}/{plat}/stra_compare_hq/ExTD/trio_validation/ttmars_trio_invalid_insdel.discpos.tsv', 'w')
    nosigs = open(f'{workdir}/{plat}/stra_compare_hq/ExTD/trio_validation/ttmars_trio_invalid_insdel.nosigs.tsv', 'w')
    inconclu = open(f'{workdir}/{plat}/stra_compare_hq/ExTD/trio_validation/ttmars_trio_invalid_insdel.inconclu.tsv', 'w')

    print(f'Chrom\tStart\tEnd\tID\tLength\tSig size Std\tSig pos Std\tSig size mean', file=dissize)
    size_outlier_svinfo = {}
    sig_annot = f'{workdir}/{plat}/stra_compare_hq/ExTD/trio_validation/ttmars_trio_invalid_insdel.annot.tsv'
    ...
    for idx, row in df_sig_annot.iterrows():
        ...
        svid, svlen, est_size, size_std, start_std = row['id'], abs(int(row['svlen'])), int(row['size_mean']), int(row['size_std']), int(row['start_diff_std'])
    ...
    ax.pie([60, 24, 9, 7, 8, 2], radius=1 - size, colors=['#cccccc', '#fdae6b', '#a1d99b', '#bcbddc', '#7fcdbb', '#c994c7'],
