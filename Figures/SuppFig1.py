...
def suppfig1a(workdir, datasets, aligners):
    ...
    sns.stripplot(data=read_ins_pcrt, x='dataset', y='pcrt', hue='aa', hue_order=aligners,
                  palette=[ALIGNERCOLOR[ele] for ele in aligners], size=7, edgecolor='black', linewidth=1, ax=axes[1])
...
def suppfig1b(workdir, datasets, aligners):
    ...
    sns.histplot(data=df_assmsvs_small, x='size', hue='svtype', bins=70, hue_order=svtype_order, palette=svtype_colors, alpha=1, ax=axes[0])
    ...
    sns.histplot(data=df_assmsvs_large, x='size', hue='svtype', bins=70, hue_order=svtype_order, palette=svtype_colors, alpha=1, ax=axes[1])
    ...
    fig, axes = plt.subplots(2, 1, figsize=(7, 5))
    sns.histplot(data=df_readsvs_small, x='size', hue='svtype', bins=70,  hue_order=svtype_order, palette=svtype_colors, alpha=1, ax=axes[0])

    axes[0].set_xlim(50, 1000)
    axes[0].set_xticks([50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000])
    axes[0].set_xticklabels(['50', '100', '200', '300', '400', '500', '600', '700', '800', '900', '1,000'], fontsize=12, rotation=60)

    axes[0].set_yticks([0, 75000, 150000])
    axes[0].set_yticklabels(['0','75,000', '150,000'], fontsize=12)

    sns.histplot(data=df_readsvs_large, x='size', hue='svtype', bins=70, hue_order=svtype_order, palette=svtype_colors, alpha=1, ax=axes[1])

    axes[1].set_xlim(1000, 10000)
    axes[1].set_xticks([1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000])
    axes[1].set_xticklabels(['1,000', '2,000', '3,000', '4,000', '5,000', '6,000', '7,000', '8,000', '9,000', '10,000'], fontsize=12, rotation=60)
...
def suppfig1c(workdir):

    assembly_counts = {region: [] for region in GENOMICREGIONS}
    read_counts = {region: [] for region in GENOMICREGIONS}

    read_insdel_counts = {'INS':{region: [] for region in GENOMICREGIONS}, 'DEL': {region: [] for region in GENOMICREGIONS}}
    assm_insdel_counts = {'INS': {region: [] for region in GENOMICREGIONS}, 'DEL': {region: [] for region in GENOMICREGIONS}}
    ...
    for idx, row in pav_sv_count.iterrows():
        dataset, region, svcount, assembler, svtype = row['dataset'], row['region'], int(row['count']), row['assembler'], row['svtype']
    ...
    for key, count_region in assm_caller_sv_region.items():
        ...
        if 'ont' in dataset:
            plat = 'ONT'
        for region, count in count_region.items():
            sv_region_info.append((caller, plat, dataset, assembler, count / caller_sv_totals[key] * 100, region, 'Assembly'))
    ...
    read_insdel_avg = {}
    for region in GENOMICREGIONS:
        read_insdel_avg[region] = [np.mean(read_insdel_counts['INS'][region]), np.mean(read_insdel_counts['DEL'][region])]

    df_read_insdel_avg = pd.DataFrame(read_insdel_avg)

    totals = [i + j + k + m for i, j, k, m in zip(df_read_insdel_avg['Tandem Repeats'], df_read_insdel_avg['Repeat Masked'], df_read_insdel_avg['Segment Dup'],df_read_insdel_avg['Simple Region'])]
    ...
    read_ax.bar(r, sd_pcrt, width=size, color=REGIONCOLORS['Segment Dup'], bottom=[i + j for i, j in zip(sim_pcrt, rm_pcrt)])
    read_ax.bar(r, uni_pcrt, width=size, color=REGIONCOLORS['Simple Region'], bottom=[i + j + k for i, j, k in zip(sim_pcrt, rm_pcrt, sd_pcrt)])

    assm_ax = axes[1]
    assm_ax.set_title('Assembly-based', fontsize=13)
    assm_insdel_avg = {}
    for region in GENOMICREGIONS:
        assm_insdel_avg[region] = [np.mean(assm_insdel_counts['INS'][region]), np.mean(assm_insdel_counts['DEL'][region])]

    df_assm_insdel_avg = pd.DataFrame(assm_insdel_avg)

    totals = [i + j + k + m for i, j, k, m in zip(df_assm_insdel_avg['Tandem Repeats'], df_assm_insdel_avg['Repeat Masked'], df_assm_insdel_avg['Segment Dup'], df_assm_insdel_avg['Simple Region'])]
    ...
    assm_ax.bar(r, sd_pcrt, width=size, color=REGIONCOLORS['Segment Dup'],bottom=[i + j for i, j in zip(sim_pcrt, rm_pcrt)])
    assm_ax.bar(r, uni_pcrt, width=size, color=REGIONCOLORS['Simple Region'],bottom=[i + j + k for i, j, k in zip(sim_pcrt, rm_pcrt, sd_pcrt)])
...
def suppfig1d(workdir, datasets):
